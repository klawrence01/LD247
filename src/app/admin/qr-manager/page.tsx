// src/app/admin/qr-manager/page.tsx
"use client";
...
export default function AdminQRManagerPage() {
  // pretend vendors — we’ll load from Supabase later
  const fakeVendors = [
    { id: "v-1", name: "Tony's Pizza", city: "new-haven" },
    { id: "v-2", name: "Metro Bagels", city: "hartford" },
  ];
  ...
  const [savedQrs, setSavedQrs] = useState<SavedQR[]>([
    {
      id: "qr-1",
      label: "Example vendor QR",
      target_url: "https://your-domain.com/vendor/join?v=v-1&city=new-haven",
      audience: "vendor",
      owner_id: "v-1",
      notes: "sample row",
  ...
  const applyQuickVendor = () => {
    ...
    const vendor = fakeVendors.find((v) => v.id === selectedVendorId);
    const city = (selectedCity || vendor?.city || "hartford").toLowerCase();

    const built = `https://your-domain.com/vendor/join?v=${selectedVendorId}&city=${encodeURIComponent(
      city
    )}`;
    ...
    setNotes(`QR auto-built for ${vendor?.name ?? "vendor"} in ${city}`);
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-neutral-900/40 border border-neutral-800 rounded-2xl p-6 mb-6">
          ...
          <p className="text-sm text-neutral-300 mb-4">
            ...
            automatically so a scan can either follow the vendor or view daily deals for that city.
          </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {savedQrs.map((qr) => {
                ...
                return (
                  ...
                  >
                    <img src={imgSrc} alt={qr.label} className="w-20 h-20 rounded-lg bg-white/5 object-contain" />
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-neutral-900 border border-neutral-700 rounded-2xl w-full max-w-md p-6 relative">
            ...
            <div className="flex flex-col items-center gap-4">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(previewUrl)}`}
                alt={previewLabel}
